          ...
          <div className="cart-body">
              ...
              {items.map(item => {
                ...
                return (
                  <div key={item.key} className="cart-item">
                    <Link to={`/product/${p.productId || p.id}`} className="cart-item-img">
                    ...
                    <div className="cart-item-info">
                      <div className="cart-item-top">
                        <div>
                          ...
                          <Link to={`/product/${p.productId || p.id}`} className="cart-item-name">{p.name}</Link>
            ...
            <div className="cart-summary">
              ...
              <div className="summary-rows">
                ...
                {shipping > 0 && <p className="ship-hint">Add ₹{(999 - subtotal).toLocaleString('en-IN')} more for free shipping</p>}
              </div>
              <div className="summary-total">
                <span>Total</span><span>₹{total.toLocaleString('en-IN')}</span>
              </div>
              <p className="coupon-hint">🎟 Apply coupon codes at checkout</p>
              <button className="checkout-btn" onClick={() => navigate('/checkout')}>
                Proceed to Checkout <ArrowRight size={16}/>
              </button>
              <Link to="/" className="continue-shopping">← Continue Shopping</Link>
              <div className="summary-assurance"><p>🔒 Secure checkout · 30-day returns</p></div>
            </div>
          </div>
        )}
      </div>
