    ...
    <section className="hero-section">
      ...
      <div className="hero-content container">
        <div className="hero-text">
          <p className="hero-eyebrow animate-fadeUp" style={{ animationDelay: '0ms' }}>Summer 2025 Collection</p>
          ...
          <p className="hero-desc animate-fadeUp" style={{ animationDelay: '160ms' }}>{slide.description}</p>
          <div className="hero-actions animate-fadeUp" style={{ animationDelay: '220ms' }}>
            <button className="hero-btn-primary" onClick={() => navigate(`/category/${slide.category}`)}>
              {slide.cta} <ArrowRight size={16}/>
            </button>
            <button className="hero-btn-ghost" onClick={() => navigate('/')}>All Collections</button>
          </div>
        </div>
...
// Fetches products from the API for a given category
function ProductSection({ categoryId }) {
  ...
  const visible = products.filter(p => {
    const matchSearch = !query || p.name.toLowerCase().includes(query.toLowerCase()) || p.brand.toLowerCase().includes(query.toLowerCase());
    const matchFilter = filter === 'all' || p.badge === filter;
      ...
      <div className="container">
        ...
        {loading ? (
          ...
          <div className="no-results">
            <p>No {cat?.label?.toLowerCase()} found{query ? ` matching "${query}"` : ''}.</p>
            <button className="tag" onClick={() => { setQuery(''); setFilter('all'); }}>Clear filters</button>
          </div>
        ) : (
          <div className="product-grid">
            {visible.slice(0, 4).map((p, i) => <ProductCard key={p._id || p.productId} product={p} delay={i * 60}/>)}
          </div>
        )}
      </div>
    </section>
    ...
    <div className="promo-banner container">
      <div className="promo-inner">
        <div className="promo-text">
          ...
          <p>30-day easy returns · COD available · Secure Payment</p>
        </div>
